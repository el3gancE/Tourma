...
<%
        ...
        try {
            ...
            if (currentTournament != null) {
                if ((currentFormat == null || currentFormat.trim().isEmpty()) && currentTournament.getFormat() != null) {
                    currentFormat = currentTournament.getFormat();
                }
                if ((currentType == null || currentType.trim().isEmpty()) && currentTournament.getTournamentType() != null) {
                    currentType = currentTournament.getTournamentType();
                }
                if ((seriesIdParam == null || seriesIdParam.trim().isEmpty()) && currentTournament.getSeriesId() != null) {
                    seriesIdParam = currentTournament.getSeriesId();
                }
                if (currentTournament.getTierName() != null && !currentTournament.getTierName().trim().isEmpty()) {
                    currentTierVal = currentTournament.getTierName().trim().toUpperCase();
                }
            }
        } catch (Exception e) {
            // Keep empty
    ...
    boolean isRollingSubTournament = (seriesIdParam != null && !seriesIdParam.trim().isEmpty());

    String formActionUrl = isRollingSubTournament
    ...
    <main class="container has-sidebar">
        ...
        <div class="form-container-box" style="margin-top: 0;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.1rem; flex-wrap: wrap; gap: 0.75rem;">
                ...
                <% if (isRollingSubTournament) { %>
                <!-- TIER CHANGER WIDGET (ONLY FOR ROLLING SUB-TOURNAMENTS) -->
                <div class="tier-switcher-box">
                    <span style="font-size: 0.8rem; font-weight: 700; color: #94a3b8; display: flex; align-items: center; gap: 0.35rem;">
                        <i class="fa-solid fa-layer-group text-gold"></i> Cấp Độ (Tier):
                    </span>
                    <div class="tier-pills-row">
                        <button type="button" class="tier-pill-btn <%= "S".equals(currentTierVal) ? "active tier-s" : "" %>" data-tier="S" onclick="changeTournamentTier('S')">
                            Tier S
                        </button>
                        <button type="button" class="tier-pill-btn <%= "A".equals(currentTierVal) ? "active tier-a" : "" %>" data-tier="A" onclick="changeTournamentTier('A')">
                            Tier A
                        </button>
                        <button type="button" class="tier-pill-btn <%= "B".equals(currentTierVal) ? "active tier-b" : "" %>" data-tier="B" onclick="changeTournamentTier('B')">
                            Tier B
                        </button>
                        <button type="button" class="tier-pill-btn <%= "C".equals(currentTierVal) ? "active tier-c" : "" %>" data-tier="C" onclick="changeTournamentTier('C')">
                            Tier C
                        </button>
                        <button type="button" class="tier-pill-btn <%= "D".equals(currentTierVal) ? "active tier-d" : "" %>" data-tier="D" onclick="changeTournamentTier('D')">
                            Tier D
                        </button>
                    </div>
                    <span id="tierSaveIndicator" style="display: none; font-size: 0.75rem; color: #34d399; font-weight: 700; margin-left: 0.25rem;">
            ...
            <form id="configureFormatForm" action="<%= formActionUrl %>" method="GET"
                onsubmit="return validateAndSubmitFormat(event)">
                <input type="hidden" name="id" value="<%= (tournamentId != null) ? tournamentId : "" %>">
                <input type="hidden" name="seriesId"
                    value="<%= (seriesIdParam != null) ? seriesIdParam : "" %>">
                <input type="hidden" id="selectedTournamentType" name="tournamentType"
                    value="<%= currentType %>">
                ...
                <input type="hidden" id="advancingSeatsCount" name="advancingSeatsCount" value="0">
                            ...
                            <div id="singleStagePanel">
                                <div class="form-group">
                                    ...
                                    <div class="format-pill-grid">
                                        <button type="button" class="format-pill-btn" id="pillSingleElim"
                                            onclick="selectFormat('SINGLE_ELIMINATION', true)">
                                            Single Elimination
                                        </button>
                                        <button type="button" class="format-pill-btn" id="pillDoubleElim"
                                ...
                                <div id="wdlPointsPanel"
                                    ...
                                    <div class="section-label-uppercase"
                                        style="margin-bottom: 0.6rem; color: var(--gold-primary);">
                                        <i class="fa-solid fa-sliders"></i> QUY TẮC CỘNG ĐIỂM & SỐ LẦN GẶP NHAU
                                    </div>
                                    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 0.6rem;">
                                        <div>
                                            <label class="form-label"
                                                style="font-size: 0.75rem; color: var(--text-muted);">Điểm Thắng</label>
                                            <input type="number" name="winPoints" class="form-control" value="3" min="0"
                                                max="10">
                                        </div>
                                        <div>
                                            <label class="form-label"
                                                style="font-size: 0.75rem; color: var(--text-muted);">Điểm Hòa</label>
                                            <input type="number" name="drawPoints" class="form-control" value="1"
                                                min="0" max="10">
                                        </div>
                                        <div>
                                            <label class="form-label"
                                                style="font-size: 0.75rem; color: var(--text-muted);">Điểm Thua</label>
                                            <input type="number" name="lossPoints" class="form-control" value="0"
                                                min="0" max="10">
                                        </div>
                                        <div>
                                            <label class="form-label"
                                                style="font-size: 0.75rem; color: var(--text-muted);">Số Lần Gặp
                                                Nhau</label>
                                            <input type="number" name="legsCount" class="form-control" value="1" min="1"
                                                max="10">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div id="multiStagePanel" style="display: none;">
                                ...
                                <!-- STAGE 1 -->
                                <div
                                    style="background: rgba(11, 13, 18, 0.7); border: 1px solid rgba(45, 212, 191, 0.25); border-radius: 12px; padding: 1.25rem; margin-bottom: 1.25rem;">
                                    <div
                                        style="font-size: 0.95rem; font-weight: 800; color: #2dd4bf; margin-bottom: 0.85rem; display: flex; align-items: center; gap: 0.5rem;">
                                        <i class="fa-solid fa-layer-group"></i> STAGE 1
                                    </div>

                                    <label class="form-label"
                                        style="font-size: 0.78rem; color: var(--text-muted); margin-bottom: 0.5rem;">
                                        Thể thức:
                                    </label>
                                    <div class="format-pill-grid" style="margin-bottom: 1rem;">
                                        <button type="button" class="format-pill-btn active" id="pillStage1RR"
                                            onclick="selectStage1Format('ROUND_ROBIN', true)">
                                            Round Robin
                                        </button>
                                        <button type="button" class="format-pill-btn" id="pillStage1GR"
                                            onclick="selectStage1Format('GROUP_STAGE', true)">
                                            Group Stage
                                        </button>
                                        <button type="button" class="format-pill-btn" id="pillStage1SE"
                                            onclick="selectStage1Format('SINGLE_ELIMINATION', true)">
                                            Single Elimination
                                        </button>
                                    ...
                                    <div id="stage1ConfigContainer"
                                        style="background: #181d29; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 10px; padding: 1rem;">
                                        ...
                                        <div id="stage1FieldsRR">
                                            <div style="margin-bottom: 0.75rem;">
                                                ...
                                                <input type="number" id="stage1AdvanceRR" name="stage1AdvanceRR"
                                                    class="form-control"
                                                    style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                    value="" min="1" placeholder="Nhập số đội đi tiếp...">
                                            </div>
                                            <div
                                                style="background: rgba(11, 13, 18, 0.6); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; padding: 0.85rem;">
                                                <div
                                                    style="font-size: 0.75rem; font-weight: 700; color: #fbbf24; margin-bottom: 0.5rem;">
                                                    <i class="fa-solid fa-sliders"></i> QUY TẮC CỘNG ĐIỂM & SỐ LẦN GẶP
                                                    NHAU
                                                </div>
                                                <div
                                                    ...
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Thắng</label>
                                                        <input type="number" id="stage1WinPointsRR"
                                                            name="stage1WinPointsRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                    ...
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Hòa</label>
                                                        <input type="number" id="stage1DrawPointsRR"
                                                            name="stage1DrawPointsRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                    ...
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Thua</label>
                                                        <input type="number" id="stage1LossPointsRR"
                                                            name="stage1LossPointsRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="0" min="0" max="10">
                                                    </div>
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Số
                                                            Lượt Gặp Nhau</label>
                                                        <input type="number" id="stage1LegsCountRR"
                                                            name="stage1LegsCountRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                        ...
                                        <div id="stage1FieldsGR" style="display: none;">
                                            <div style="margin-bottom: 0.75rem;">
                                                <label class="form-label"
                                                    style="font-size: 0.78rem; color: var(--text-muted);">Tổng Số Đội Đi
                                                    Tiếp <span style="color: #f43f5e;">*</span></label>
                                                <input type="number" id="stage1AdvanceGR" name="stage1AdvanceGR"
                                                    class="form-control"
                                                    style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                    value="" min="1" placeholder="Nhập tổng số đội đi tiếp...">
                                            </div>
                                            <div
                                                style="background: rgba(11, 13, 18, 0.6); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; padding: 0.85rem;">
                                                <div
                                                ...
                                                <div
                                                    ...
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Thắng</label>
                                                        <input type="number" id="stage1WinPointsGR"
                                                            name="stage1WinPointsGR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="3" min="0" max="10">
                                                    </div>
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Hòa</label>
                                                        <input type="number" id="stage1DrawPointsGR"
                                                            name="stage1DrawPointsGR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="1" min="0" max="10">
                                                    </div>
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Thua</label>
                                                        <input type="number" id="stage1LossPointsGR"
                                                            name="stage1LossPointsGR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="0" min="0" max="10">
                                                    </div>
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Số
                                                            Lượt Gặp Nhau</label>
                                                        <input type="number" id="stage1LegsCountGR"
                                                            name="stage1LegsCountGR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="1" min="1" max="10">
                                                    </div>
                                                </div>
                                        ...
                                        <div id="stage1FieldsSE" style="display: none;">
                                            <label class="form-label"
                                                style="font-size: 0.78rem; color: var(--text-muted);">Số Đội Đi Tiếp
                                                <span style="color: #f43f5e;">*</span></label>
                                            <input type="number" id="stage1AdvanceSE" name="stage1AdvanceSE"
                                                class="form-control"
                                                style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                value="" min="1" placeholder="Nhập số đội đi tiếp...">
                                        </div>
                                        ...
                                        <div id="stage1FieldsSwiss" style="display: none;">
                                            <div
                                                style="display: flex; align-items: center; justify-content: space-between; background: rgba(45, 212, 191, 0.08); border: 1px solid rgba(45, 212, 191, 0.3); border-radius: 8px; padding: 0.75rem 1rem;">
                                                <div style="color: #cbd5e1; font-size: 0.82rem; font-weight: 600;">
                                                    <i class="fa-solid fa-lock"
                                                        style="color: #2dd4bf; margin-right: 0.4rem;"></i>
                                                    Cấu hình cố định Swiss System:
                                                </div>
                                                <div style="display: flex; gap: 0.5rem;">
                                                    <span class="stage-badge"
                                                        style="background: rgba(255,255,255,0.08); color: #ffffff; border-color: rgba(255,255,255,0.2);">16
                                                        Đội Tham Gia</span>
                                                    <span class="stage-badge"
                                ...
                                <div
                                    ...
                                    <div class="format-pill-grid" style="margin-bottom: 0.5rem;">
                                        <button type="button" class="format-pill-btn active" id="pillStage2SE"
                                            onclick="selectStage2Format('SINGLE_ELIMINATION', true)">
                                            Single Elimination
                                        </button>
                                        <button type="button" class="format-pill-btn" id="pillStage2DE"
                                            onclick="selectStage2Format('DOUBLE_ELIMINATION', true)">
                                            Double Elimination
                                        </button>
                                        <button type="button" class="format-pill-btn" id="pillStage2RR"
                                            onclick="selectStage2Format('ROUND_ROBIN', true)">
                                            Round Robin
                                        </button>
                                    </div>

                                    <!-- DYNAMIC SUB-PANEL FOR STAGE 2 FORMAT CONFIG -->
                                    <div id="stage2ConfigContainer"
                                        style="display: none; background: #181d29; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 10px; padding: 1rem; margin-top: 0.85rem;">
                                                ...
                                                <div
                                                    ...
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Thắng</label>
                                                        <input type="number" id="stage2WinPointsRR"
                                                            name="stage2WinPointsRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="3" min="0" max="10">
                                                    </div>
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Hòa</label>
                                                        <input type="number" id="stage2DrawPointsRR"
                                                            name="stage2DrawPointsRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="1" min="0" max="10">
                                                    </div>
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Điểm
                                                            Thua</label>
                                                        <input type="number" id="stage2LossPointsRR"
                                                            name="stage2LossPointsRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="0" min="0" max="10">
                                                    </div>
                                                    <div>
                                                        <label class="form-label"
                                                            style="font-size: 0.72rem; color: var(--text-muted);">Số
                                                            Lượt Gặp Nhau</label>
                                                        <input type="number" id="stage2LegsCountRR"
                                                            name="stage2LegsCountRR" class="form-control"
                                                            style="background: #0b0d12; color: #ffffff; border-color: rgba(255, 255, 255, 0.15);"
                                                            value="1" min="1" max="10">
                                                    </div>
                                                </div>
                ...
                <div id="formatLockedModalBackdrop" class="tourma-modal-backdrop"
                    onclick="if(event.target === this) closeFormatLockedModal();">
                    <div class="tourma-modal-card" style="max-width: 480px; border-color: rgba(244, 63, 94, 0.4);"
                        ...
                        <div class="modal-header-bar" style="border-bottom: 1px solid rgba(244, 63, 94, 0.2);">
                            ...
                            <button type="button" class="modal-close-btn" onclick="closeFormatLockedModal()"
                                title="Đóng">
                        ...
                        <div class="modal-body-content" style="padding: 1.25rem 1rem;">
                            <div
                                ...
                                Giải đấu này đang diễn ra (đã có trận đấu có kết quả). Bạn không thể thay đổi thể thức
                                thi đấu khi giải đấu đang diễn ra.
                            </div>
                        ...
                        <div class="modal-footer-bar" style="display: flex; justify-content: flex-end; gap: 0.65rem;">
                            <button type="button" class="btn btn-mint" onclick="closeFormatLockedModal()"
                                style="font-size: 0.8rem; font-weight: 700; padding: 0.45rem 1.25rem;">
                                Đã Hiểu & Đóng
                            </button>
                        </div>
                    </div>
                ...
                <div id="legsChangeModalBackdrop" class="tourma-modal-backdrop"
                    onclick="if(event.target === this) closeLegsChangeModal();">
                    <div class="tourma-modal-card" style="max-width: 480px; border-color: rgba(244, 63, 94, 0.4);"
                        onclick="event.stopPropagation();">
                        <div class="modal-header-bar" style="border-bottom: 1px solid rgba(244, 63, 94, 0.2);">
                            <div class="modal-header-title"
                            ...
                            <button type="button" class="modal-close-btn" onclick="closeLegsChangeModal()"
                                title="Hủy bỏ">
                                <i class="fa-solid fa-xmark"></i>
                            </button>
                        </div>
                        ...
                        <div class="modal-body-content" style="padding: 1.25rem 1rem;">
                            <div
                                style="background: rgba(244, 63, 94, 0.08); border: 1px solid rgba(244, 63, 94, 0.2); border-radius: 8px; padding: 0.85rem; margin-bottom: 1rem; color: #cbd5e1; font-size: 0.88rem; line-height: 1.5;">
                                <strong style="color: #f43f5e;">⚠️ Cảnh báo quan trọng:</strong><br>
                                Việc thay đổi <strong>Số lần gặp nhau</strong> sẽ tạo lại cấu trúc giải và <strong
                                    style="color: #ffffff;">RESET toàn bộ lịch thi đấu cùng kết quả các trận</strong>
                                của bạn.
                            </div>
                            <p style="color: #94a3b8; font-size: 0.8rem; margin: 0;">
                        ...
                        <div class="modal-footer-bar" style="display: flex; justify-content: flex-end; gap: 0.65rem;">
                            <button type="button" class="btn btn-secondary" onclick="closeLegsChangeModal()"
                            ...
                            <button type="button" class="btn"
                                ...
                                onclick="confirmLegsChangeAndSubmit()">
                ...
                <script>
                    ...
                    var currentTier = "<%= currentTierVal %>";
                    ...
                    function changeTournamentTier(tier) {
                        ...
                        currentTier = tier;
                        var inp = document.getElementById('selectedTierName');
                        if (inp) inp.value = tier;

                        // Update UI buttons
                        var btns = document.querySelectorAll('.tier-pill-btn');
                        btns.forEach(function (btn) {
                            var t = btn.getAttribute('data-tier');
                            btn.classList.remove('active', 'tier-s', 'tier-a', 'tier-b', 'tier-c', 'tier-d');
                            if (t === tier) {
                                btn.classList.add('active', 'tier-' + tier.toLowerCase());
                            }
                        });
                        ...
                        // Save to localStorage & AJAX persist to database
                        if (tournamentId) {
                            localStorage.setItem('tourma_tier_' + tournamentId, tier);

                            var ctx = "${pageContext.request.contextPath}";
                            fetch(ctx + "/rolling/update-tier?tournamentId=" + encodeURIComponent(tournamentId) + "&tierName=" + encodeURIComponent(tier), {
                                method: "POST"
                            }).then(function (res) {
                                return res.json();
                            }).then(function (data) {
                    ...
                    window.addEventListener('DOMContentLoaded', function () {
                        ...
                        // Restore Tier if saved in localStorage (only for Rolling Sub-tournaments)
                        if (isRollingSubTournament && tournamentId) {
                            var savedTier = localStorage.getItem('tourma_tier_' + tournamentId);
